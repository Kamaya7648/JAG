...
function Sidebar({ active,setActive }) {
  const menus = [
    ...
    { name: "Profile",icon: UserCircle,isComponent: true }
  ];
    ...
    <aside className="sidebar">
      ...
      <nav className="menu">
        {menus.map((item) => {
          ...
          return (
            <button key={item.name} className={`menu-item ${active === item.name ? "active" : ""}`} onClick={() => setActive(item.name)}>
              <span className="icon">
                {item.isComponent ? <Icon size={18} /> : <img src={item.icon} alt={item.name} />}
              </span>
